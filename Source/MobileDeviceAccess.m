...
- (BOOL)copyRemoteFile:(NSString*)path1 toLocalFile:(NSString*)path2
{
	...
	if ([self ensureConnectionIsOpen]) {
		...
		if ([fm fileExistsAtPath:path2]) {
            ...
            {
                NSLog(@"Can't open file: %@!", path1);
            }
		}
	}
    
    if (self.lasterror != nil) {
        NSLog(@"the last error: %@!", self.lasterror);
    }
	return result;
}
...
- (BOOL)copyRemoteFile:(NSString*)path1 toLocalDir:(NSString*)path2
{
    ...
    NSFileManager *fm = [NSFileManager defaultManager];
    long suffixCount = 0;
    NSString *newToFile = dest;
    NSString *suffix;
    
    while ([fm fileExistsAtPath:newToFile]) {
        suffixCount ++;
        suffix = [NSString stringWithFormat:@"(%ld)", suffixCount] ;
        newToFile = [dest stringByAppendingString:suffix];
    }
    
    return [self copyRemoteFile:path1 toLocalFile:newToFile];
}
